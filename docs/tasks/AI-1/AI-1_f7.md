...
**Обновление 2 (23.04.2026 13:40):**

Исправлена ошибка при выполнении `make shell`:

**Проблема:**
```
agent.py: error: the following arguments are required: --task
```

**Причина:**
`ENTRYPOINT` в Dockerfile всегда запускал `agent.py`, даже при попытке открыть shell.

**Решение:**
1. Заменен `ENTRYPOINT` на `CMD` в Dockerfile
2. Обновлена команда `runagent` в Makefile для явного указания `python aider/agent.py`
3. Удалено устаревшее поле `version: '3.8'` из docker-compose.yml

Теперь `make shell` корректно открывает bash в контейнере.
