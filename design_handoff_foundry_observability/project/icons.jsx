...
const I = {
  ...
  Discord:  (p) => <svg {...p} viewBox="0 0 24 24" className={`ico ${p.className||''}`}><path d="M19 5a16 16 0 0 0-4-1l-.3.6a13 13 0 0 0-5.4 0L9 4a16 16 0 0 0-4 1C2 9 1.5 13 2 17c1.6 1.2 3.2 2 4.7 2.5l1-1.5c-.9-.3-1.7-.7-2.4-1.2.2-.1.4-.3.6-.4a11 11 0 0 0 10.2 0l.6.4c-.7.5-1.5.9-2.4 1.2l1 1.5c1.5-.5 3.1-1.3 4.7-2.5.6-4.7-.3-8.6-3-12zM9 15c-.9 0-1.7-.9-1.7-2s.7-2 1.7-2 1.7.9 1.7 2-.7 2-1.7 2zm6 0c-.9 0-1.7-.9-1.7-2s.7-2 1.7-2 1.7.9 1.7 2-.7 2-1.7 2z"/></svg>,
  Webhook:  (p) => <svg {...p} viewBox="0 0 24 24" className={`ico ${p.className||''}`}><path d="M9 8a3 3 0 1 1 5.5 1.5l3 5"/><path d="M14 17a3 3 0 1 1-3-3l3-5"/><path d="M5 17a3 3 0 1 1 3 3h6"/></svg>,
  Pin:      (p) => <svg {...p} viewBox="0 0 24 24" className={`ico ${p.className||''}`}><path d="M9 4l6 0M12 4v8M7 12h10l-2 4H9zM12 16v4"/></svg>,
  Bolt:     (p) => <svg {...p} viewBox="0 0 24 24" className={`ico ${p.className||''}`}><path d="M13 2L4 14h7l-1 8 9-12h-7z"/></svg>,
  Stop:     (p) => <svg {...p} viewBox="0 0 24 24" className={`ico ${p.className||''}`}><rect x="6" y="6" width="12" height="12" rx="1"/></svg>,
  Refresh:  (p) => <svg {...p} viewBox="0 0 24 24" className={`ico ${p.className||''}`}><path d="M3 12a9 9 0 1 0 3-6.7L3 8"/><path d="M3 3v5h5"/></svg>,
  ChevronRight: (p) => <svg {...p} viewBox="0 0 24 24" className={`ico ${p.className||''}`}><path d="M9 6l6 6-6 6"/></svg>,
  ChevronLeft: (p) => <svg {...p} viewBox="0 0 24 24" className={`ico ${p.className||''}`}><path d="M15 6l-6 6 6 6"/></svg>,
  Loader:   (p) => <svg {...p} viewBox="0 0 24 24" className={`ico ${p.className||''}`} style={{...(p.style||{}), animation: 'spinner 1.2s linear infinite'}}><path d="M12 3v3M12 18v3M5.6 5.6l2.1 2.1M16.3 16.3l2.1 2.1M3 12h3M18 12h3M5.6 18.4l2.1-2.1M16.3 7.7l2.1-2.1"/></svg>,
  Sparkle:  (p) => <svg {...p} viewBox="0 0 24 24" className={`ico ${p.className||''}`}><path d="M12 3l2 6 6 2-6 2-2 6-2-6-6-2 6-2z"/></svg>,
};
